...
void GameManager::updateMusic(float dt, GameStateManager::State gameState) {
    AudioManager& audio = AudioManager::getInstance();
    audio.setMusicDimmed(gameState == GameStateManager::State::Pause);

    switch (gameState) {
        case GameStateManager::State::Title:
            audio.playMusic(MusicID::Title, 1.5f);
            break;
        case GameStateManager::State::Tutorial:
            audio.playMusic(MusicID::Game, 1.5f);
            break;
        case GameStateManager::State::Playing:
            if (EntityManager::getInstance().getTime() >= 120.f) {
                audio.playMusic(MusicID::Wave, 1.f);
            }
            else {
                audio.playMusic(MusicID::Game, 1.5f);
            }
        ...
        case GameStateManager::State::Pause:
            break;
        case GameStateManager::State::GameOver:
            audio.playMusic(MusicID::GameOver, 1.f);
            break;
        case GameStateManager::State::Victory:
            audio.playMusic(MusicID::Victory, 1.f);
            break;
        case GameStateManager::State::Exit:
            audio.stopMusic(0.4f);
            break;
    }
