...
Player::Player()
    ...
    healthRecoveryRate(5.f), staminaRecoveryRate(50.f),
...
void Player::handleInput(BuildingManager& buildManager) {
    ...
    if (keyboardInput) {
        ...
        bool isMoving = (movementVector.x != 0.f || movementVector.y != 0.f);
        // sprint -> if button pressed while moving, make player move 1.5 times faster
        if (input.isKeyDown(sf::Keyboard::Key::LShift) && isMoving) {
...
void Player::updatePosition(sf::Vector2f movementVec, float dt) {
    ...
    if (fixedMovementVec.x != 0.f || fixedMovementVec.y != 0.f) {
        ...
        AudioManager::getInstance().play("playerWalkFootstep", false);
...
void Player::onCollision(float damage, sf::Vector2f damageOrigin) {
    ...
    if (fromCollision > invincibility && targetState != AnimationController::State::Death) {
        ...
        if (health > damage) { 
            health -= damage; 
            AudioManager::getInstance().play("playerHurt", false);
        }
        else {
            health = 0;
            // isAlive = false;
            ...
            targetState = AnimationController::State::Death;
            AudioManager::getInstance().play("playerDeath", false);
        }
    }
}
...
void Player::onAttack() {
    ...
    AudioManager::getInstance().play("playerAttack", true);
}
