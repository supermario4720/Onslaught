...
Button::Button(const sf::Vector2f& size, const sf::Vector2f& position, const std::string& textString, sf::Font& font)
    :text(font), sprite(TextureManager::getInstance().getTexture("Button")), isMouseAlreadyOver(false), isActive(true)
{
    box.setSize(size);
    ...
    if (isSquare) {
        ...
        // used to scale button to provided size
...
bool Button::isMouseOver(sf::RenderWindow& window) {
    ...
    if (mouseOver && !isMouseAlreadyOver) {
        AudioManager::getInstance().play("ButtonHover", true);
...
bool Button::isClicked(sf::RenderWindow& window) {
    ...
    if (clicked) {
        AudioManager::getInstance().play("ButtonClick", true);
        if(isSquare) {
