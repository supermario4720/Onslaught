...
PlayerProgression EntityManager::getPlayerProgression() const {
	if (!player) return PlayerProgression();
	return player->getStatusManager().getProgression();
}

bool EntityManager::spendPlayerStatusPoint(PlayerStatType stat) {
	if (!player) return false;
	return player->getStatusManager().spendStatusPoint(stat);
}
