...
void InventoryManager::updateSlotTextures(float screenW, float screenH) {
    int i = 0;
    const int columns = 3;
    const float slotSpacing = 72.f;
    const float startX = screenW * 0.20f;
    ...
    for (auto& slot : inventory) {
        ...
        float row = static_cast<float>(i / columns);
        float column = static_cast<float>(i % columns);
        sf::Vector2f slotPos = { startX + column * slotSpacing, startY + row * slotSpacing };
        slot.emptySlot.setPosition(slotPos);
        slot.sprite.setPosition(slotPos);
        i++;
        ...
        slot.qtyText.setPosition({ slotPos.x, slotPos.y + 20.f } );
    }
}
