...
void EnemyManager::spawnEnemy(sf::Vector2f position) {
	std::shared_ptr<Enemy> newEnemy = Enemy::create(position);
...
void EnemyManager::enemySpawnerUpdate(float dt) {
...
sf::Vector2f EnemyManager::randomSpawnPosition() {
	...
	while (attempts < 10) {
        ...
        // check if position is outside forbidden zone around town (±100 from 0,0)
        bool insideForbiddenZone = (spawnPosition.x >= -100.f && spawnPosition.x <= 100.f && 
                                    spawnPosition.y >= -100.f && spawnPosition.y <= 100.f);
        
        // return if no collision and outside forbidden zone
		if ( !CollisionManager::getInstance().checkAllCollision(placeholder) && !insideForbiddenZone ) {
...
int EnemyManager::returnEnemyCount() {
	return enemies.size();
}